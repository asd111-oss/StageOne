  ...
  (function () {
    ...
    const PEEK = 16;
    const activeRange = 0.85;
    const seg = activeRange / Math.max(n - 1, 1);
    ...
    const update = () => {
      ...
      cards.forEach((card, i) => {
        ...
        let y = restY;
        if (i > 0) {
          const local = Math.min(Math.max((P - (i - 1) * seg) / seg, 0), 1);
          y = H * (1 - local) + restY * local;
        }
        let scale = 1;
        if (i < n - 1) {
          const cover = Math.min(Math.max((P - i * seg) / seg, 0), 1);
